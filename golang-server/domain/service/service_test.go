...
func Test_packService_Addlist(t *testing.T) {
	type fields struct {
		repository contracts.PackRepository
	}
	type args struct {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				repository: func() contracts.PackRepository {
					packService := &mocks.PackRepository{}
					packService.On("AddList", []int{1, 2, 3, 4}).Return()
					return packService
				}(),
			},
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := p.Addlist(tt.args.packs); (err != nil) != tt.wantErr {
...
func Test_packService_RemoveList(t *testing.T) {
	type fields struct {
		repository contracts.PackRepository
	}
	type args struct {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				repository: func() contracts.PackRepository {
					packService := &mocks.PackRepository{}
					packService.On("RemoveList", []int{1, 2, 3, 4}).Return()
					return packService
				}(),
			},
			args: args{
